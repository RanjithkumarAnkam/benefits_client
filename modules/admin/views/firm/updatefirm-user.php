
<section class="page-content  padding-0">
	<div class="page-content-inner">

		<!-- clients -->
		<div class="dashboard-container">

			<div class="header-main" id="header">
				<span class="font-18">Add/Edit Firm User</span>
			</div>

			 
			
			
			 
			<div class="col-md-12">
		
		<?= $this->render('_form-user')?>
		    
          </div>

		</div>
		<script>
		$(document).ready(function(){
		var html = '<span class="font-18">Update Firm User</span><a href="/admin/firm/firm-user" class="btn btn-success pull-right"> Add</a>';
		//	var button = '<div class="row margin-top-15"><a class="btn btn-success pull-right">Update</a></div>';
			var lielement = '<li class="" aria-disabled="false"><a role="menuitem">Update</a></li>';
			$('#header').html('');
			$('#header').append(html);
		//	$('.update-button').html();
			 $('#save_button').html('');
			$('.actions>ul').append(lielement);
		});
		</script>
	</div>
</section>

