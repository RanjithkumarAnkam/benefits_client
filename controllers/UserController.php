		...
		if (isset ( $_POST ['admin_user_id'] ) && isset ( $_POST ['status'] )) {
			...
			try {
				if ($_POST ['status'] == 'A1') {
					...
					$adminusers = AdminUsers::find()->where(['admin_id'=>$admin_user_id ])->One ();
					$users = User::find()->where(['user_id'=>$adminusers->user_id])->One();
		...
		if (isset ( $_POST ['firm_user_id'] ) && isset ( $_POST ['status'] )) {
			...
			try {
				if ($_POST ['status'] == 'A1') {
					...
					$users = User::find()->where(['user_id'=>$firmusers->user_id])->One();
		...
		if (isset ( $_POST ['client_user_id'] ) && isset ( $_POST ['status'] )) {
			...
			try {
				if ($_POST ['status'] == 'A1') {
					...
					$users = User::find()->where(['user_id'=>$clientusers->user_id])->One();
	...
	public function actionChangefirmstatus() {
		if (isset ( $_POST ['firm_id'] ) && isset ( $_POST ['status'] )) {
			...
			try {
				if ($_POST ['status'] == 'A1') {
					//print_r('A1');die();
					$firms = Firms::find()->where(['firm_id'=>$firm_id])->One();
					$firms->is_active = 1;
		...
		try {
			...
			if ($model->load ( Yii::$app->request->post () )) {
				...
				if ($model->save ()) {
					...
					$pricing_details = SystemPricing::find()->where(['module_id'=>$module_id,'package_type'=>$package_id])->One();
					...
					{
						print_r($model_company_subscription->errors);
						die();
				...
				{
					throw new \Exception('Please client subscription.');
				}
				}
			}
		} catch ( \Exception $e ) {
